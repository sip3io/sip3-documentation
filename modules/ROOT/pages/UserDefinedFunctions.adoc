...
As mentioned in the previous section, the main purpose of any SIP3 UDF is assigning `String` or `Boolean` attribute to a SIP3 packet according to its business logic. We call such attributes User-Defined Attributes (UDAs). If you follow best practices described below UDAs may bring your user experience with SIP3 to the next level.
