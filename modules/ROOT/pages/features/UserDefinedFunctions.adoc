...
We call such attributes User-Defined Attributes (UDAs). Each UDA by default is available in Search query and as Tag in metrics.

But in some cases write UDA as Tag in metrics will be enough. To configure how UDA will be processed there are 3 processing mode:

- _Database_ - with this mode attribute will be saved in database and available in SIP3 UI Search query.
- _Options_ - enables writing options for attribute with *String* type. It will be available for autocompletion in SIP3 UI Search query.
- _Metrics_ - enables recording metrics for attribute.

Enable only certain mode for UDA is possible by adding prefix to name of attribute. Prefix separated from name with delimiter `:` and may contain:

- `d` for _Database_
- `o` for _Options_
- `m` for _Metrics_

Empty prefix means that attribute is used in processing only and will not appear in Search query, Database or Metrics.

Let's take a closer look at processing modes:
[source,text]
----
user_agent                  // <1>
...
<1> `user_agent` - UDA with default behaviour: available for Search in UI with autocomplete options and as Tag in metrics.
<2> `do:operation_system` - UDA `operation_system` will be saved in database and available in Search query as `sip.operation_system` with options for select.
<3> `d:x_session_id` - `x_session_id` will be saved in database and available in Search query as `sip.x_session_id`.
<4> `m:robo_call` - `robo_call` will be available only as Tag for Metrics.
<5> `:wangiri_candidate` - `wangiri_candidate` will be available only in xref:features/UserDefinedFunctions.adoc[User-Defined Functions] and never be saved.
