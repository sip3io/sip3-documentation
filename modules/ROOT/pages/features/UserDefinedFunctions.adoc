...
User-Defined Functions can be stored in the file system or in MongoDB.
...
[source,groovy]
----
package SipMessageUdfVerticle

import io.vertx.core.AbstractVerticle
import io.sip3.salto.ce.udf.UdfExecutor
import org.slf4j.LoggerFactory

public class SipMessageUdfVerticle extends AbstractVerticle {

    def logger = LoggerFactory.getLogger(UdfExecutor.class)
...
public class SipMessageUdfVerticle extends AbstractVerticle {
    ...
    public void start() {
        vertx.eventBus().localConsumer("sip_message_udf", { event ->
            ...
            def matcher = (from_header =~ /1(\d*)_(\d*)/)
            if (matcher) {
                packet['attributes']['caller'] = matcher[0][1]
                packet['attributes']['d:suffix'] = matcher[0][2]
            }
...
[source,groovy]
----
package PacketUdfVerticle

import io.vertx.core.AbstractVerticle
import io.sip3.salto.ce.udf.UdfExecutor
import org.slf4j.LoggerFactory

public class PacketUdfVerticle extends AbstractVerticle {
...
[source,groovy]
----
package SipCallUdfVerticle

import io.vertx.core.AbstractVerticle
import io.sip3.salto.ce.udf.UdfExecutor
import org.slf4j.LoggerFactory

public class SipCallUdfHandler extends AbstractVerticle {
...
== UDF management interface

In case of UDF stored in MongoDB there is a management interface in SIP3 Hoof. It helps users to create/update/delete UDFs without accessing FS.

image::UdfManagementInterface.png[UDF Management interface]

- `UDF Name` - the name of the UDF
- `Created at` - date and time of the UDF creation
- `Updated at` - date and time of the UDF update
- `Enabled` - is UDF enabled
- `Executors` - executor list where UDF will be deployed on.
- `Deployments` - UDF deployment states for each executor

- `Actions` - available actions for the UDF:
* image:EnableDisableIcon.png[Enable/Disable] - enable or disable UDF
* image:EditIcon.png[Edit] - edit UDF
* image:CopyIcon.png[Copy] - copy UDF
* image:DeleteIcon.png[Delete] - completely remove UDF from SIP3.
