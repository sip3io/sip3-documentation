...
<3> If the amount of returned IPs equals 250(which is a bulk size) we will do the next API call using the control ID immediately.
...
To make the use case more interesting and show you real data we agreed to deploy the User-Defined Function from above to one of our customers.

As soon as it was deployed we built a metric dashboard using `blocked` attribute. Here is what we've got:

image::DashboardForCallAttemptsBlockedByApiban.png[Dashboard For Call Attempts Blocked By APIBAN]

As you can see there is a good amount of fraud attempts blocked by the list of IPs retrieved from APIBAN API. Let's see what are those attempts:

image::SearchForCallAttemptsBlockedByApiban.png[Search For Call Attempts Blocked By APIBAN]

Now let's see how many blacklisted IPs are sending data to our VoIP servers:

image::ShowIpListBlockedByApiban.png[Show IP List Blocked By APIBAN]

TODO: Conclusions about SIP3 providing you full visibility in fighting with VOIP fraud